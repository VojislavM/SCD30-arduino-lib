...
const uint8_t SCD30::tableCRCMSB[] = {
...
const uint8_t SCD30::tableCRCLSB[] = {
...
void SCD30::begin()
{
    ...
    Serial1.begin(BAUD_RATE); //intended for use with Hardware Serial
...
{
    uint8_t byteCounter = 0;
    sendCommand(SCD30_READ_HOLDING_REGISTERS, SCD30_SET_MEASUREMENT_INTERVAL, 1);
    delay(20);
    ...
    while (Serial1.available() > 0)
    {
        ...
        if (byteCounter == 3) interval = incoming << 8; //measurement interval data is stored in bytes 4 and 5
        if (byteCounter == 4) interval |= incoming;
...
{
    uint8_t byteCounter = 0;
    sendCommand(SCD30_READ_HOLDING_REGISTERS, SCD30_SET_AUTOMATIC_SELFCALIBRATION, 1);
    delay(20);
    ...
    {
        uint8_t incoming = Serial1.read();
        if (byteCounter == 3) ASC = incoming << 8; //ASC setting is stored in bytes 4 and 5
        if (byteCounter == 4) ASC |= incoming;
...
{
    uint8_t byteCounter = 0;
    sendCommand(SCD30_READ_HOLDING_REGISTERS, SCD30_SET_FORCED_RECALIBRATION, 1);
    delay(20);
    ...
    {
        uint8_t incoming = Serial1.read();
        if (byteCounter == 3) FRC = incoming << 8; //FRC value is stored in bytes 4 and 5
        if (byteCounter == 4) FRC |= incoming;
...
{
    uint8_t byteCounter = 0;
    sendCommand(SCD30_READ_HOLDING_REGISTERS, SCD30_SET_TEMPERATURE_OFFSET, 1);
    delay(20);
    ...
    {
        uint8_t incoming = Serial1.read();
        if (byteCounter == 3) temperature = incoming << 8; //temperature offset value is stored in bytes 4 and 5
        if (byteCounter == 4) temperature |= incoming;
...
{
    uint8_t byteCounter = 0;
    sendCommand(SCD30_READ_HOLDING_REGISTERS, SCD30_SET_ALTITUDE_COMPENSATION, 1);
    delay(20);
    ...
    {
        uint8_t incoming = Serial1.read();
        if (byteCounter == 3) altitude = incoming << 8; //altitude compensation value is stored in bytes 4 and 5
        if (byteCounter == 4) altitude |= incoming;
...
void SCD30::readMeasurement()
{
    ...
    delay(30); //set the delay to allow sensor to respond
...
//argument is desired value when writing data 
//or number of holding registers to read when reading data
void SCD30::sendCommand(uint8_t functionCode, uint16_t address, uint16_t argument)
{
    ...
    data[6] = crcLSB; //set last two bytes of request
    data[7] = crcMSB;
...
{
    delay(20); //we set a delay so the sensor has enough time to respond
    while (Serial1.available() > 0)
    {
        Serial1.read();
    }
...
{
    uint8_t byteCounter = 0;
    sendCommand(SCD30_READ_HOLDING_REGISTERS, SCD30_READ_FIRMWARE_VERSION, 1);
    delay(15);

    while (Serial1.available() > 0)
    {
        uint8_t incoming = Serial1.read();
        if (byteCounter == 3) firmwareVersion[0] = incoming; //firmware version data is stored in bytes 4 and 5
        if (byteCounter == 4) firmwareVersion[1] = incoming;
...
{
    sendCommand(SCD30_WRITE_SINGLE_HOLDING_REGISTER, SCD30_SOFT_RESET, 1);
    clearBuffer();
}